...
import {
  ...
  AlertCircle, CheckCircle2, Loader2, Hash, Brain, Eye, Pencil
} from "lucide-react";

// ═══════════════════════════════════════════════════════════════
//  CONFIG — Groq API (100% FREE | 30 req/min | No credit card)
//  Get key → https://console.groq.com/keys
// ═══════════════════════════════════════════════════════════════
const API_KEY = import.meta.env.VITE_GROQ_API_KEY || "";
const AI_MODEL = "llama-3.3-70b-versatile";
const hasKey = API_KEY.length > 10 && API_KEY !== "your-groq-key-here";

// ─── Constants ───
const AI_PROVIDERS = [
  { id: "groq", name: "Groq (Free)", icon: "⚡", desc: "Llama 3.3 70B — FREE, 30 req/min", local: false },
  { id: "ollama", name: "Ollama (Local)", icon: "🦙", desc: "Run locally with Ollama", local: true },
  { id: "claude", name: "Claude", icon: "🟣", desc: "Anthropic Claude API", local: false },
  { id: "gemini", name: "Gemini", icon: "🟢", desc: "Google Gemini API", local: false },
  { id: "custom", name: "Custom OpenAI", icon: "🔧", desc: "Any OpenAI-compatible endpoint", local: false },
];
...
Return ONLY valid JSON with exactly this structure (no markdown, no backticks, no extra text before or after):
{"sections":"A thorough 2-4 sentence summary of the main topics discussed, referencing specific subjects and participants","decisions":"Specific decisions that were made during the meeting, citing who agreed to what","actions":"Concrete action items with responsible persons and deadlines if mentioned"}`,
  ...
  standup: {
    ...
    prompt: `You are an expert meeting analyst. Analyze this standup/sync meeting transcript in standup format. Be specific, only reference what was actually discussed. Do NOT invent information.

Return ONLY valid JSON (no markdown, no backticks, no extra text):
{"sections":"What was accomplished/discussed — reference specific updates each person shared","decisions":"What the team agreed to work on next — specific tasks and priorities mentioned","actions":"Blockers identified and who is responsible for resolving them"}`,
  },
  technical: {
    name: "Technical Review", desc: "Architecture decisions, code changes, follow-ups",
    prompt: `You are an expert technical meeting analyst. Analyze this technical meeting transcript. Be precise about technical details. Do NOT invent information.

Return ONLY valid JSON (no markdown, no backticks, no extra text):
{"sections":"Technical topics and architecture discussions, referencing specific technologies or components","decisions":"Technical decisions made — approaches chosen and why","actions":"Technical follow-ups — who implements what, code reviews needed"}`,
  },
  sales: {
    name: "Sales Call", desc: "Pain points, objections, next steps",
    prompt: `You are an expert sales call analyst. Analyze this client call transcript. Be specific about client needs. Do NOT invent information.

Return ONLY valid JSON (no markdown, no backticks, no extra text):
{"sections":"Client pain points and needs expressed, with specific references","decisions":"Objections raised, agreements on pricing/scope/timeline","actions":"Next steps — follow-ups, proposals, demos, with responsible persons"}`,
  },
};
  ...
  if (ext === "srt") {
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes("-->")) {
        ...
        if (tl.length) { const sp = tl[0].match(/^([A-Za-z\s]+?):\s*(.*)/); segs.push(sp ? { time: time.substring(0, 7), speaker: sp[1].trim(), text: sp[2] + (tl.length > 1 ? " " + tl.slice(1).join(" ") : "") } : { time: time.substring(0, 7), speaker: "Speaker", text: tl.join(" ") }); }
      }
    }
  } else if (ext === "vtt") {
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes("-->")) {
        const tm = lines[i].match(/(\d{2}:\d{2}[:.]\d{2})/); const time = tm ? tm[1].replace(".", ":").substring(0, 7) : "00:00"; const tl = [];
        let j = i + 1; while (j < lines.length && lines[j].trim() && !lines[j].includes("-->")) { tl.push(lines[j].trim()); j++; }
        if (tl.length) { const vt = tl[0].match(/^<v\s+([^>]+)>(.*)/); const ct = tl[0].match(/^([A-Za-z\s]+?):\s*(.*)/);
          if (vt) segs.push({ time, speaker: vt[1].trim(), text: vt[2] }); else if (ct) segs.push({ time, speaker: ct[1].trim(), text: ct[2] }); else segs.push({ time, speaker: "Speaker", text: tl.join(" ") }); }
      }
    }
  } else if (ext === "json") {
    try { const d = JSON.parse(content); const arr = Array.isArray(d) ? d : d.segments || d.transcript || d.results || [];
      arr.forEach((it, i) => segs.push({ time: it.time || it.timestamp || fmt(i * 15), speaker: it.speaker || it.name || "Speaker", text: it.text || it.content || "" }));
    } catch { segs.push({ time: "00:00", speaker: "Speaker", text: content.substring(0, 500) }); }
  } else {
    lines.forEach((line, i) => {
      const ts = line.match(/^\[?(\d{1,2}:\d{2}(?::\d{2})?)\]?\s*(.*)/); const sp = line.match(/^([A-Za-z0-9\s._-]+?):\s*(.*)/);
      if (ts) { const inner = ts[2].match(/^([A-Za-z0-9\s._-]+?):\s*(.*)/); segs.push(inner ? { time: ts[1], speaker: inner[1].trim(), text: inner[2] } : { time: ts[1], speaker: "Speaker", text: ts[2] }); }
      else if (sp) segs.push({ time: fmt(i * 10), speaker: sp[1].trim(), text: sp[2] });
      else if (line.trim().length > 2) segs.push({ time: fmt(i * 10), speaker: "Speaker", text: line.trim() });
    });
  }
  return segs.length > 0 ? segs : [{ time: "00:00", speaker: "Speaker", text: content.substring(0, 1000) }];
};

// ═══════════════════════════════════════════════════════════════
//  REAL AI SUMMARIZATION — Groq API (FREE)
//  Model: Llama 3.3 70B | 30 req/min | 14,400 req/day
//  Get key → https://console.groq.com/keys
// ═══════════════════════════════════════════════════════════════
const summarizeWithAI = async (transcript, templateId) => {
  ...
  // Call Groq via Vite proxy (avoids CORS)
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) throw new Error("Invalid Groq API key.\nGet a free one: https://console.groq.com/keys");
    if (res.status === 429) throw new Error("Rate limited. Groq free tier: 30 req/min.\nWait 30 seconds and retry.");
    if (res.status === 413) throw new Error("Transcript too long for this model.\nTry a shorter transcript or different template.");
  ...
  try {
    ...
    throw new Error("Model returned unparseable response. Try again.");
  }
};

// ─── LocalStorage ───
const SK = { meetings: "meetily_meetings", settings: "meetily_settings", sidebar: "meetily_sidebar", last: "meetily_last_active" };
const load = (k, fb) => { try { const s = localStorage.getItem(k); return s ? JSON.parse(s) : fb; } catch { return fb; } };
const save = (k, v) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch {} };
const getInitMeetings = () => { const s = load(SK.meetings, null); if (s?.length > 0) return s; save(SK.meetings, SAMPLE_MEETINGS); return SAMPLE_MEETINGS; };
const DEF = { aiProvider: "groq", ollamaModel: "llama3.2:3b", whisperModel: "small", language: "en", micDevice: "default", systemDevice: "default", theme: "dark", autoSave: true, gpu: true };
...
const Modal = ({ open, onClose, children, title, wide }) => {
  ...
  return (<div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
    ...
    <div onClick={(e) => e.stopPropagation()} className={`relative bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl ${wide ? "w-full max-w-3xl" : "w-full max-w-lg"} max-h-[90vh] overflow-hidden flex flex-col`}>
      ...
      <div className="overflow-y-auto flex-1 p-5">{children}</div></div></div>);
};
    ...
    <div className="flex-1 min-w-0">
      {tab === "ai" && (<div className="space-y-4"><p className="text-sm text-gray-400 mb-3">AI provider for meeting summaries</p>
        <div className={`p-3 rounded-xl border ${hasKey ? "bg-green-500/10 border-green-500/20" : "bg-amber-500/10 border-amber-500/20"}`}>
          <p className={`text-xs flex items-center gap-1.5 font-medium ${hasKey ? "text-green-400" : "text-amber-400"}`}>
            {hasKey ? <><CheckCircle2 size={12} /> Groq API key active — Llama 3.3 70B (FREE)</> : <><AlertCircle size={12} /> Free key needed → <a href="https://console.groq.com/keys" target="_blank" rel="noopener" className="underline hover:text-amber-300">console.groq.com/keys</a></>}</p></div>
        <div className="grid grid-cols-1 gap-2">{AI_PROVIDERS.map((p) => (
          <button key={p.id} onClick={() => setSettings((s) => ({ ...s, aiProvider: p.id }))}
            ...
            <span className="text-xl">{p.icon}</span><div className="flex-1"><div className="text-sm font-medium text-white flex items-center gap-2">{p.name}
              ...
              {p.id === "groq" && <span className="text-[10px] px-1.5 py-0.5 bg-orange-500/20 text-orange-400 rounded-full font-medium">FREE</span>}
      ...
      {tab === "transcription" && (<div className="space-y-4"><p className="text-sm text-gray-400 mb-3">Transcription model (processed locally)</p>
        ...
        <div className="mt-4"><label className="text-xs text-gray-400 font-medium block mb-1.5">Language</label>
          <select value={settings.language} onChange={(e) => setSettings((s) => ({ ...s, language: e.target.value }))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500">
      ...
      {tab === "audio" && (<div className="space-y-4"><p className="text-sm text-gray-400 mb-3">Configure audio input devices</p>
        <div><label className="text-xs text-gray-400 font-medium flex items-center gap-1.5 mb-2"><Mic size={13} /> Microphone</label>
          <select value={settings.micDevice} onChange={(e) => setSettings((s) => ({ ...s, micDevice: e.target.value }))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500">
            <option value="default">Default - Built-in Microphone</option><option value="airpods">AirPods Pro</option><option value="usb">USB Condenser Mic</option></select></div>
        <div><label className="text-xs text-gray-400 font-medium flex items-center gap-1.5 mb-2"><Speaker size={13} /> System Audio</label>
          <select value={settings.systemDevice} onChange={(e) => setSettings((s) => ({ ...s, systemDevice: e.target.value }))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500">
      ...
      {tab === "general" && (<div className="space-y-4">
        ...
        <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-xl"><div className="flex items-center gap-2 text-green-400 text-sm font-medium mb-1"><Shield size={14} /> Privacy First</div>
          <p className="text-xs text-green-400/70">All data stored in your browser's localStorage. Nothing leaves your device except summary requests to Groq.</p></div>
        <div className="p-3 bg-gray-800 rounded-xl border border-gray-700"><div className="flex items-center justify-between">
          <div><p className="text-sm text-gray-300">Clear All Local Data</p><p className="text-xs text-gray-500 mt-0.5">Remove all meetings and reset settings</p></div>
          <button onClick={() => { if (window.confirm("Delete all meetings and reset?")) { Object.values(SK).forEach((k) => localStorage.removeItem(k)); window.location.reload(); }}}
            className="px-3 py-1.5 rounded-lg text-xs font-medium bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 transition-colors">Clear</button></div></div>
      </div>)}
    </div></div></Modal>);
};
...
const ImportModal = ({ open, onClose, onImport }) => {
  ...
  return (<Modal open={open} onClose={onClose} title="Import Audio File"><div className="space-y-4">
    <p className="text-sm text-gray-400">Import an audio file to create a new meeting</p>
    <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
      ...
      {file ? (<div className="space-y-2"><CheckCircle2 size={36} className="mx-auto text-green-400" /><p className="text-sm text-white font-medium">{file.name}</p><p className="text-xs text-gray-500">{(file.size / 1048576).toFixed(1)} MB</p></div>)
        : (<div className="space-y-3"><Upload size={36} className="mx-auto text-gray-500" /><p className="text-sm text-gray-300">Drop audio file or click to browse</p><p className="text-xs text-gray-600">MP4, M4A, WAV, MP3, FLAC, OGG, AAC</p></div>)}</div>
    <div className="flex gap-3 justify-end">
      <button onClick={() => { setFile(null); onClose(); }} className="px-4 py-2 rounded-xl text-sm text-gray-400 hover:text-white">Cancel</button>
      <button onClick={() => { if (file) { onImport(file.name); setFile(null); onClose(); } }} disabled={!file} className={`px-5 py-2 rounded-xl text-sm font-medium ${file ? "bg-indigo-500 hover:bg-indigo-600 text-white" : "bg-gray-700 text-gray-500 cursor-not-allowed"}`}>Import & Transcribe</button>
    </div></div></Modal>);
};

// ─── Summary Config Modal ───
const SummaryModal = ({ open, onClose, onGenerate }) => {
  ...
  return (<Modal open={open} onClose={onClose} title="Generate AI Summary"><div className="space-y-4">
    <div className="flex items-center gap-3 p-3 bg-gray-800 rounded-xl border border-gray-700">
      <span className="text-lg">⚡</span><div><div className="text-sm font-medium text-white">Groq — Llama 3.3 70B</div><div className="text-xs text-gray-500">30 req/min · 14,400 req/day</div></div>
      <span className="ml-auto text-[10px] px-2 py-0.5 bg-orange-500/20 text-orange-400 rounded-full font-medium">FREE</span></div>
    <div className="p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl">
      <p className="text-xs text-indigo-300 flex items-center gap-1.5"><Sparkles size={12} /> Your actual transcript content will be analyzed and summarized in real-time.</p></div>
    ...
    <div className="flex gap-3 justify-end pt-2">
      ...
      <button onClick={() => { onGenerate(template); onClose(); }} className="px-5 py-2 rounded-xl text-sm font-medium bg-indigo-500 hover:bg-indigo-600 text-white flex items-center gap-2"><Sparkles size={14} /> Generate Summary</button>
    </div></div></Modal>);
};
...
const TranscriptUploadModal = ({ open, onClose, onUpload }) => {
  ...
  return (<Modal open={open} onClose={() => { reset(); onClose(); }} title="Upload Transcript for Summarization" wide><div className="space-y-4">
    <div className="flex items-start gap-3 p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl">
      ...
      <div><p className="text-sm text-indigo-300 font-medium">Upload an existing transcript</p>
        <p className="text-xs text-indigo-400/60 mt-0.5">From Zoom, Teams, Otter, or plain text — Llama 3.3 70B will analyze and summarize for free.</p></div></div>
    {!file ? (
      ...
      <div className="flex items-center gap-3 p-3 bg-gray-800 rounded-xl border border-gray-700">
        ...
        <button onClick={reset} className="p-1.5 rounded-lg hover:bg-gray-700 text-gray-500 hover:text-red-400"><X size={15} /></button></div>
      ...
      <div><button onClick={() => setShowP(!showP)} className="flex items-center gap-2 text-xs text-gray-400 hover:text-white font-medium mb-2"><Eye size={13} /> {showP ? "Hide" : "Show"} Preview {showP ? <ChevronUp size={13} /> : <ChevronDown size={13} />}</button>
        {showP && (<div className="max-h-48 overflow-y-auto rounded-xl border border-gray-700 bg-gray-800/50">
          {parsing ? (<div className="flex items-center justify-center gap-2 py-8 text-gray-500"><Loader2 size={16} className="animate-spin" /> Parsing...</div>)
            : (<div className="divide-y divide-gray-800">{segs.slice(0, 15).map((s, i) => (<div key={i} className="flex gap-3 px-3 py-2 text-xs"><span className="text-gray-600 font-mono w-14 shrink-0">{s.time}</span><span className="text-indigo-400 font-semibold w-20 shrink-0 truncate">{s.speaker}</span><span className="text-gray-300 flex-1">{s.text}</span></div>))}
              {segs.length > 15 && <div className="px-3 py-2 text-xs text-gray-600 text-center">+{segs.length - 15} more</div>}</div>)}</div>)}</div>
      <div className="flex items-center justify-between p-3 bg-gray-800 rounded-xl border border-gray-700">
        <div className="flex items-center gap-2.5"><Sparkles size={15} className="text-indigo-400" /><div><p className="text-sm text-gray-200 font-medium">Auto-generate AI summary</p><p className="text-xs text-gray-500 mt-0.5">Via Groq Llama 3.3 70B (Free)</p></div></div>
      ...
      <div className="grid grid-cols-3 gap-3">{[{ l: "Segments", v: segs.length, i: Hash }, { l: "Speakers", v: [...new Set(segs.map((s) => s.speaker))].length, i: Users }, { l: "Words", v: segs.reduce((a, s) => a + s.text.split(" ").length, 0).toLocaleString(), i: FileText }].map(({ l, v, i: I }) => (
        <div key={l} className="p-2.5 bg-gray-800/50 rounded-xl border border-gray-700/50 text-center"><I size={14} className="mx-auto text-gray-500 mb-1" /><p className="text-base font-semibold text-white">{v}</p><p className="text-[10px] text-gray-500">{l}</p></div>))}</div>
    </div>)}
    <div className="flex gap-3 justify-end pt-2 border-t border-gray-800">
      <button onClick={() => { reset(); onClose(); }} className="px-4 py-2 rounded-xl text-sm text-gray-400 hover:text-white">Cancel</button>
      <button onClick={() => { if (file && segs.length) { onUpload({ title: title || file.name, segments: segs, autoSummarize: autoSum }); reset(); onClose(); } }} disabled={!file || !segs.length}
        className={`px-5 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 ${file && segs.length ? "bg-indigo-500 hover:bg-indigo-600 text-white shadow-lg shadow-indigo-500/20" : "bg-gray-700 text-gray-500 cursor-not-allowed"}`}>
        {autoSum ? <><Sparkles size={14} /> Upload & Summarize</> : <><Upload size={14} /> Upload</>}</button>
    </div></div></Modal>);
};
...
export default function App() {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => load(SK.sidebar, false));
  ...
  const [settings, setSettings] = useState(() => load(SK.settings, DEF));
  ...
  // Persist
  useEffect(() => { save(SK.meetings, meetings); }, [meetings]);
  useEffect(() => { save(SK.settings, settings); }, [settings]);
  useEffect(() => { save(SK.sidebar, sidebarCollapsed); }, [sidebarCollapsed]);
  useEffect(() => { if (activeMeeting) save(SK.last, activeMeeting.id); }, [activeMeeting]);
  useEffect(() => { const id = load(SK.last, null); if (id) { const f = meetings.find((m) => m.id === id); if (f) { setActiveMeeting(f); setActiveTab("transcript"); } } }, []);
  ...
  useEffect(() => {
    ...
    const ph = ["Let me share my thoughts on this.", "I think we should prioritize the UX improvements.", "Last quarter's metrics show promising growth.", "We need to address the tech debt.", "Can everyone review the mockups by Friday?", "Customer feedback has been overwhelmingly positive."];
    const iv = setInterval(() => { setLiveTranscript((p) => [...p, { time: fmt(elapsed), speaker: ["Sarah", "Mike", "Lisa", "Alex"][Math.floor(Math.random() * 4)], text: ph[Math.floor(Math.random() * ph.length)] }]); }, 4000 + Math.random() * 3000);
  ...
  const stopRecording = () => { setRecording(false); const m = { id: `m${Date.now()}`, title: `Meeting ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`, date: new Date().toISOString(), duration: elapsed, status: "completed", source: "recording", transcript: liveTranscript, summary: null }; setMeetings((p) => [m, ...p]); setActiveMeeting(m); setElapsed(0); };
  const handleImport = (fn) => { const m = { id: `m${Date.now()}`, title: fn.replace(/\.[^.]+$/, ""), date: new Date().toISOString(), duration: Math.floor(Math.random() * 3600) + 600, status: "completed", source: "audio-import", transcript: [{ time: "00:00", speaker: "Speaker 1", text: "Imported audio transcript." }], summary: null }; setMeetings((p) => [m, ...p]); setActiveMeeting(m); };
  const handleTranscriptUpload = ({ title, segments, autoSummarize }) => { const m = { id: `m${Date.now()}`, title, date: new Date().toISOString(), duration: segments.length * 12, status: "completed", source: "transcript-upload", transcript: segments, summary: null }; setMeetings((p) => [m, ...p]); setActiveMeeting(m); setActiveTab("transcript"); if (autoSummarize) setTimeout(() => doSummary(m.id, "default", segments), 600); };

  // Rename
  const startRename = (id, t) => { setRenamingId(id); setRenameValue(t); };
  const commitRename = () => { if (renamingId && renameValue.trim()) { setMeetings((p) => p.map((m) => m.id === renamingId ? { ...m, title: renameValue.trim() } : m)); if (activeMeeting?.id === renamingId) setActiveMeeting((p) => p ? { ...p, title: renameValue.trim() } : p); } setRenamingId(null); setRenameValue(""); };
  const cancelRename = () => { setRenamingId(null); setRenameValue(""); };

  // Summarize
  const doSummary = async (id, template, to) => {
    const tr = to || meetings.find((m) => m.id === id)?.transcript;
    if (!tr?.length) { setGenError("No transcript to summarize."); return; }
    setGenerating(true); setGenError(null); setActiveTab("summary"); setGenProgress("Sending to Groq...");
    try { setGenProgress("Llama 3.3 70B analyzing..."); const s = await summarizeWithAI(tr, template);
      setMeetings((p) => p.map((m) => m.id === id ? { ...m, summary: s } : m)); setActiveMeeting((p) => p?.id === id ? { ...p, summary: s } : p);
    } catch (e) { setGenError(e.message); } finally { setGenerating(false); setGenProgress(""); }
  };
  ...
  const badge = (s) => { if (s === "transcript-upload") return { t: "Transcript", c: "text-purple-400 bg-purple-500/15 border-purple-500/30" }; if (s === "audio-import") return { t: "Imported", c: "text-cyan-400 bg-cyan-500/15 border-cyan-500/30" }; return null; };
  const Bars = ({ level }) => (<div className="flex items-end gap-0.5 h-4">{[0,1,2,3].map((i) => (<div key={i} className="w-[3px] rounded-full bg-indigo-400 transition-all duration-100" style={{ height: `${Math.max(15, (level > i / 4 ? level * 100 : 15))}%`, opacity: level > i / 4 ? 1 : 0.3 }} />))}</div>);
  ...
  return (
    <div className="h-screen w-full bg-gray-950 text-white flex flex-col overflow-hidden" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <header className="h-13 border-b border-gray-800/80 flex items-center px-4 gap-3 shrink-0 bg-gray-950/90 backdrop-blur-md z-20">
        ...
        {recording && (<div className="flex items-center gap-2 px-3 py-1.5 bg-red-500/15 border border-red-500/30 rounded-full animate-pulse"><div className="w-2 h-2 rounded-full bg-red-500" /><span className="text-xs font-medium text-red-400">{fmt(elapsed)}</span><Bars level={liveLevel} /></div>)}
        <button onClick={() => setSettingsOpen(true)} className="p-2 rounded-lg hover:bg-gray-800 text-gray-400 hover:text-white"><Settings size={17} /></button>
        <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${hasKey ? "bg-green-500/10 border border-green-500/20" : "bg-yellow-500/10 border border-yellow-500/20"}`}>
          {hasKey ? <><Zap size={12} className="text-green-400" /><span className="text-[10px] text-green-400 font-medium">FREE</span></> : <><AlertCircle size={12} className="text-yellow-400" /><span className="text-[10px] text-yellow-400 font-medium">NO KEY</span></>}</div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={`${sidebarCollapsed ? "w-14" : "w-72"} border-r border-gray-800/80 flex flex-col shrink-0 bg-gray-950 transition-all duration-300`}>
          <div className="p-3 space-y-1.5">
            {!sidebarCollapsed ? (<>
              ...
              <div className="flex gap-1.5">
                <button onClick={() => setImportOpen(true)} className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-xs font-medium border border-gray-700 text-gray-400 hover:bg-gray-800 hover:text-white"><FileAudio size={13} /> Audio</button>
                <button onClick={() => setTranscriptUploadOpen(true)} className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-xs font-medium border border-purple-500/30 text-purple-400 hover:bg-purple-500/10 bg-purple-500/5"><FileText size={13} /> Transcript</button></div>
          ...
          {!sidebarCollapsed && (<div className="px-3 mb-2"><div className="relative"><Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-500" />
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search..." className="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg pl-8 pr-3 py-1.5 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500/50" /></div></div>)}
          <div className="flex-1 overflow-y-auto px-2">
            {!sidebarCollapsed && (<div className="space-y-0.5"><p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider px-2 py-2">Meetings</p>
              {filtered.map((m) => { const b = badge(m.source); const isR = renamingId === m.id; return (
                <div key={m.id} onClick={() => { if (!isR) { setActiveMeeting(m); setActiveTab("transcript"); setGenError(null); } }}
                  ...
                  <div className="flex items-start justify-between gap-1"><div className="flex-1 min-w-0">
                    {isR ? (<input ref={renameRef} type="text" value={renameValue} onChange={(e) => setRenameValue(e.target.value)}
                      onKeyDown={(e) => { if (e.key === "Enter") commitRename(); if (e.key === "Escape") cancelRename(); }} onBlur={commitRename} onClick={(e) => e.stopPropagation()}
                      ...
                      {b && <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium border shrink-0 ${b.c}`}>{b.t}</span>}
                      {m.summary && <CheckCircle2 size={10} className="text-green-400 shrink-0" />}</div>)}
                  ...
                  {!isR && (<div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 shrink-0">
                    <button onClick={(e) => { e.stopPropagation(); startRename(m.id, m.title); }} className="p-1 rounded hover:bg-indigo-500/20 text-gray-500 hover:text-indigo-400" title="Rename"><Pencil size={12} /></button>
                    <button onClick={(e) => { e.stopPropagation(); deleteMeeting(m.id); }} className="p-1 rounded hover:bg-red-500/20 text-gray-500 hover:text-red-400" title="Delete"><Trash2 size={12} /></button></div>)}</div>
                </div>); })}
              {!filtered.length && <p className="text-xs text-gray-600 text-center py-8">No meetings found</p>}
            </div>)}
          </div>
          <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="p-2 border-t border-gray-800/80 text-gray-500 hover:text-white hover:bg-gray-800/50 flex justify-center">
          ...
          <div className="border-b border-gray-800/80 px-5 py-3 flex items-center gap-3 shrink-0">
            {recording ? (
              <div className="flex items-center gap-3 flex-1">
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500 animate-pulse" /><span className="text-sm font-semibold text-red-400">Recording</span><span className="text-sm text-gray-400 font-mono">{fmt(elapsed)}</span></div><div className="flex-1" />
                <button onClick={() => setPaused(!paused)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-700">{paused ? <><Play size={13} /> Resume</> : <><Pause size={13} /> Pause</>}</button>
                <button onClick={stopRecording} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30"><Square size={13} /> Stop</button></div>
            ) : activeMeeting ? (
              <div className="flex items-center gap-3 flex-1"><div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  {renamingId === activeMeeting.id ? (
                    <input type="text" value={renameValue} onChange={(e) => setRenameValue(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") commitRename(); if (e.key === "Escape") cancelRename(); }} onBlur={commitRename} autoFocus
                  ...
                  ) : (<h1 onClick={() => startRename(activeMeeting.id, activeMeeting.title)} className="text-base font-semibold text-white truncate cursor-pointer hover:text-indigo-300 group flex items-center gap-1.5" title="Click to rename">
                    {activeMeeting.title}<Pencil size={12} className="text-gray-600 group-hover:text-indigo-400 shrink-0" /></h1>)}
                  {(() => { const b = badge(activeMeeting.source); return b ? <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium border shrink-0 ${b.c}`}>{b.t}</span> : null; })()}</div>
                <div className="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
                  ...
                  <span className="flex items-center gap-1"><MessageSquare size={11} /> {activeMeeting.transcript?.length || 0}</span>
                  <span className="flex items-center gap-1"><Users size={11} /> {[...new Set(activeMeeting.transcript?.map((t) => t.speaker) || [])].length}</span></div>
              </div>
              <button onClick={() => setSummaryModalOpen(true)} disabled={generating} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400 border border-indigo-500/30">
                {generating ? <Loader2 size={13} className="animate-spin" /> : <Sparkles size={13} />} {generating ? "Generating..." : "AI Summary"}</button></div>
            ) : (<div className="flex-1 text-center"><p className="text-sm text-gray-500">Select a meeting or start recording</p></div>)}
          </div>

          {(recording || activeMeeting) && (<div className="border-b border-gray-800/80 px-5 flex shrink-0">{["transcript", "summary"].map((t) => (
          ...
          <div className="flex-1 overflow-y-auto">
            {!recording && !activeMeeting ? (
              <div className="h-full flex items-center justify-center"><div className="text-center max-w-md space-y-5">
                ...
                <div><h2 className="text-lg font-semibold text-white mb-1">Ready to Record</h2><p className="text-sm text-gray-500">Start a meeting, import audio, or upload a transcript.</p></div>
                <div className="flex gap-3 justify-center flex-wrap">
                  <button onClick={startRecording} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium bg-indigo-500 hover:bg-indigo-600 text-white shadow-lg shadow-indigo-500/20"><Mic size={15} /> Start Recording</button>
                  <button onClick={() => setImportOpen(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium border border-gray-700 text-gray-300 hover:bg-gray-800"><FileAudio size={15} /> Import Audio</button>
                  <button onClick={() => setTranscriptUploadOpen(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium border border-purple-500/30 text-purple-400 hover:bg-purple-500/10 bg-purple-500/5"><FileText size={15} /> Upload Transcript</button></div>
                {!hasKey && (<div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl max-w-sm mx-auto"><p className="text-xs text-amber-400 text-center">Get your <b>free</b> Groq API key at <a href="https://console.groq.com/keys" target="_blank" rel="noopener" className="underline font-medium">console.groq.com/keys</a></p></div>)}
              ...
              <div className="p-5 space-y-1">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => handleCopy(curT.map((t) => `[${t.time}] ${t.speaker}: ${t.text}`).join("\n"))} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-gray-800 border border-gray-700/50">
                    {copied ? <><Check size={12} className="text-green-400" /> Copied</> : <><Copy size={12} /> Copy</>}</button></div>
                {!curT.length ? (<div className="flex flex-col items-center justify-center py-20 text-gray-600"><Mic size={28} className="mb-3 opacity-50" /><p className="text-sm">{recording ? "Listening..." : "No transcript"}</p></div>)
                  : (<div className="space-y-1">{curT.map((t, i) => (<div key={i} className="flex gap-3 py-2 px-3 rounded-lg hover:bg-gray-800/40">
                    <span className="text-[11px] text-gray-600 font-mono w-14 shrink-0 pt-0.5">{t.time}</span>
                    <div className="flex-1"><span className="text-xs font-semibold text-indigo-400 mr-2">{t.speaker}</span><span className="text-sm text-gray-300">{t.text}</span></div></div>))}
              ...
              <div className="p-5">
                {generating ? (
                  <div className="flex flex-col items-center justify-center py-20 space-y-4">
                    ...
                    <div className="w-48 h-1.5 bg-gray-800 rounded-full overflow-hidden"><div className="h-full bg-indigo-500 rounded-full animate-pulse" style={{ width: "60%" }} /></div></div>
                ) : genError ? (
                  <div className="flex flex-col items-center justify-center py-16 space-y-4">
                    ...
                    <button onClick={() => { setGenError(null); setSummaryModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium bg-indigo-500 hover:bg-indigo-600 text-white"><RotateCcw size={14} /> Try Again</button></div>
                ) : curS ? (
                  <div className="space-y-5">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-semibold text-white flex items-center gap-2"><Sparkles size={14} className="text-indigo-400" /> AI Summary</h3>
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleCopy(`## Summary\n${curS.sections}\n\n## Decisions\n${curS.decisions}\n\n## Actions\n${curS.actions}`)} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-gray-800 border border-gray-700/50">
                          {copied ? <Check size={12} className="text-green-400" /> : <Copy size={12} />} Markdown</button>
                        <button onClick={() => setSummaryModalOpen(true)} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-gray-800 border border-gray-700/50"><RotateCcw size={12} /> Regenerate</button></div></div>
                    <div className="p-2.5 bg-orange-500/5 border border-orange-500/15 rounded-lg"><p className="text-[11px] text-orange-400/70 flex items-center gap-1.5"><Brain size={11} /> Groq Llama 3.3 70B (Free) · {activeMeeting?.transcript?.length} segments analyzed</p></div>
                    {[{ t: "Section Summary", c: curS.sections, cl: "indigo" }, { t: "Key Decisions", c: curS.decisions, cl: "amber" }, { t: "Action Items", c: curS.actions, cl: "green" }].map(({ t, c, cl }) => (
                      <div key={t} className={`p-4 rounded-xl border bg-gray-800/30 ${cl === "indigo" ? "border-indigo-500/20" : cl === "amber" ? "border-amber-500/20" : "border-green-500/20"}`}>
                        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${cl === "indigo" ? "text-indigo-400" : cl === "amber" ? "text-amber-400" : "text-green-400"}`}>{t}</h4>
                        <p className="text-sm text-gray-300 leading-relaxed">{c}</p></div>))}</div>
                ) : (
                  <div className="flex flex-col items-center justify-center py-20 space-y-4">
                    ...
                    <p className="text-sm text-gray-400">Click "AI Summary" to generate from your transcript</p>
                    <button onClick={() => setSummaryModalOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium bg-indigo-500 hover:bg-indigo-600 text-white"><Sparkles size={14} /> Generate Summary</button></div>
                )}</div>
            )}
          </div>
      ...
      <SummaryModal open={summaryModalOpen} onClose={() => setSummaryModalOpen(false)} onGenerate={generateSummary} />
